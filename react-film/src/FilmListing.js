  ...
  render() {
    ...
    if(this.state.filter === "all"){
      ...
      allFilms = this.props.films.map( (film, index) => ( 
        <FilmRow film={film} key={film.id} onFaveToggle={() => this.props.onFaveToggle(film)} isFave={check_faves.includes(film.id)} onFilmRowToggle={() => this.props.onFilmRowToggle(film)} />
      ))
